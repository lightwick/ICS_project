...
for iTotal = 1:iteration
    ...
    for SNR_idx = 1 : length(EsN0)
        ...
        W_MMSE = inv(H' * H + Nt / EsN0(SNR_idx) * eye(Nt)) * H';
        W_ZF = inv(H' * H) * H';
        
        %% SM Detection
        x2_c = Real2Complex(x2_r);
        z = W_ZF * ReceivedSignal_c;

        frobenius_norm = zeros(length(SM_Candidates), 1);
        match_point = TimeFrame;

        for i1 = 1 : length(SM_Candidates)
            [BookIndex, WordIndex] = transmit2index(SM_Candidates(i1, :), a);
            for TimeSlot = 1:Np
                ApplyingAntenna = codebook{BookIndex(TimeSlot)}{WordIndex(TimeSlot)};
                frobenius_norm(i1) = frobenius_norm(i1) + norm(z(ApplyingAntenna ,TimeSlot), 'fro')^2;
                % it's orthogonal so we could possibly take that into account to make a more efficient algorithm
            end
        end
        diff = abs(match_point-frobenius_norm);
        [~, idx] = min(diff);
        ...
        % for TimeSlot = 1:TimeFrame
        %     RegardedSymbol(:, TimeSlot) = y(Detected_TSM(:, TimeSlot), TimeSlot);
        % end
        
        % Decoding
        % EuclideanDistance = pagenorm(Candidate_y - ReceivedSignal, 'fro').^2;
        % EuclideanDistance = pagenorm(CandidateSymbol - Complex2Real(RegardedSymbol), 'fro').^2;
