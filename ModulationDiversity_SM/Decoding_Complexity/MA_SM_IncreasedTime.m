...
%         EndNorm = pagenorm(Final, 'fro');
        ...
        FirstFinal = pagemtimes(Second, y(:, 1));
        FirstEndNorm = pagenorm(FirstFinal, 'fro');
        
        %% Second Time Slot
        SecondFinal = pagemtimes(Second, y(:, 2));
        ...
        diff = sum(abs(y(:, 1)-Candidate_y).^2, 1);
        [~, idx_1] = min(diff);

        diff = sum(abs(y(:, 2)-Candidate_y).^2, 1);
        [~, idx_2] = min(diff);
        
        DetectedBinary = de2bi([Candidates(idx_1, :) Candidates(idx_2, :)], log2(M));
...
TotalTransmitBits = (Np * log2(M) * 2 + log2(c)) * iteration;
