...
Candidates_SM = zeros(Nt, 1, length(Candidates)*c);
...
for i1=1:length(codebook)
    for i2=1:length(ModulatedCandidates)
        Candidates_SM(codebook(i1,:), :, count) = ModulatedCandidates(i2, :) * theta(i1);
...
for iTotal = 1:iteration
    ...
    for SNR_idx = 1 : length(EsN0)
        ...
        Candidate_y = pagemtimes(H, Candidates_SM);
        Norm = pagenorm(y-Candidate_y, 'fro');

        [val, idx] = min(Norm);
        DetectedSignal = Candidates(mod(idx-1, length(Candidates))+1, :);
        ...
        TransmitError = sum((de2bi(TransmittedCodeword_idx-1,log2(c))~=de2bi(TransmitCandidate(idx), log2(c))), 'all');
