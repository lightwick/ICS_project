...
for iTotal = 1:iteration
    ...
    for SNR_idx = 1 : length(EsN0)
        ...
        [minVal, minIndex]= min(x1, [], 'all');
        [detected_x1, ~] = ind2sub(size(x1), minIndex);

        [minVal, minIndex]= min(x2, [], 'all');
        [detected_x2, ~] = ind2sub(size(x2), minIndex);
        ...
        detected_l = detected_l-1; % Due to the starting index being 1, 1 is subtracted
        if Nt~=2
            DetectedTransmitterBinary = de2bi(detected_l, log2(c), 'left-msb');
        end
        
        if Nt~=2
            ErrorCount = sum((SignalBinary~=DetectedBinary), 'all') + sum((TransmitterBinary~=DetectedTransmitterBinary), 'all');
        else
        ...
        found = false;
        for ii_test=1:c
            if isequal(H_l(:, 1, ii_test)*Candidates(SignalSequence(1,1)+1) + H_l(:, 2, ii_test)*Candidates(SignalSequence(2,1)+1),y)
                found = true;
            end
        end
