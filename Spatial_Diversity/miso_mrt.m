function [BitErrorCount, SignalErrorCount] = miso_mrt(SignalSequence, Noise, SignalBinary,  M, H)
    % TODO: Normalization
    Nt = size(H,2);
    Nr = size(H,1);
    ...
    w = H';
    TransmitSymbol = w*SignalSequence;
    ReceivedSymbol = H*TransmitSymbol + Noise;
    
%     y = ReceivedSymbol;
    
    FrobSquared = H*H'; % equivalent to norm(H,'fro')^2
    DetectedSignal = qamdemod(ReceivedSymbol/FrobSquared, M);
