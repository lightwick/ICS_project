...
%     clc
    ...
    if nargin < 1
        ...
        %aac = matlab.lang.correction.AppendArgumentsCorrection('16');
        %error(aac, 'MATLAB:notEnoughInputs', 'Not enough input arguments.')   
        M=16
    end
    
    if M<4
       error('Error. M must be at least 4')
    elseif mod(sqrt(M),2)~=0
       error('Error. sqrt(M) must be the power of 2')
    end
    ...
    for iTotal = 1 : NumberIteration
        ...
        for indx_EbN0 = 1 : length(EbN0)
            ...
            % MLD Receiver; ZF MLD 차이점이 여기에서는 없는게 맞는가??
